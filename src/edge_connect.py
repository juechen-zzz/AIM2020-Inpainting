...
class EdgeConnect():
    ...
    def test(self):
        ...
        for items in test_loader:
            ...
            images, images_gray, edges, masks = self.cuda(*items)
            # test_image=self.postprocess(images)[0].cpu().numpy()

            # test_image_2=self.to_tensor(test_image.astype(np.uint8))
            # test_image_3=test_image_2[np.newaxis,:]
            # images=test_image_3.cuda()
            index += 1
            ...
            # inpaint with edge model / joint model
                ...
                if self.detectBox(masks):
                    #complete your work for box mask
                    # if images.shape[2]<512 or images.shape[3]<512:
                    #     outputs=self.ensemble(images,masks,edges,images_gray)
                    #     print("It is small box")
                    # else:
                    outputs=self.resize_to_merge(images,edges,masks,images_gray)# img, edges, masks,gray
                    # print("It is box")
                elif images.shape[2]<1024 or images.shape[3]<1024:
                    outputs=self.ensemble(images,masks,edges,images_gray)#picture,picture_mask,picture_edges,picture_gray
                    # print("It is small image")
                else:
                    #complete your work for other mask 
                    outputs=self.crop_to_merge(images,edges,masks,images_gray)
                    # print("other mask")
            ...
            path = os.path.join(self.results_path, name).replace('_with_holes','')
            print(index, name)
    ...
    def resize_to_merge(self, img, edges, masks,gray):
        ...
        outputs1 = self.inpaint_model(img128, edges128, masks128)
        ...
        outputs2 = self.inpaint_model(img256, edges256, masks256)
        #Processing 512 size
        ...
        outputs3 = self.inpaint_model(img512, edges512, masks512)
        ...
        outputs = outputs1_origin * 0.3 + outputs2_origin * 0.3 + outputs3_origin * 0.4
    ...
    def ensemble(self,picture,picture_mask,picture_edges,picture_gray):
        ...
        outputs=self.network(picture,picture_mask,picture_edges,picture_gray)
...
# #Patch cutting part
    ...
    def network(self,patch,patch_mask,edge_patch,gray_patch):
        h,w,_=patch.shape
        h1,w1=0,0
        if h%4!=0 or w%4!=0:
        ...
        # print("edges: ",edges.shape)
        # print("patch_mask: ",patch_mask.shape)
        output = self.inpaint_model(patch, edges, patch_mask)
        ...
        return output
    def crop_to_merge(self, img, edges, masks,gray):
        ...
        for i in range(len(CropPatch_img)):
            CropPatch_img[i]=self.network(CropPatch_img[i],CropPatch_masks[i],CropPatch_edges[i],CropPatch_gray[i])#patch,patch_mask,edge_patch,gray_patch
...
#Methods for cropping Patch:
def cropSingle(img, size=1024,overlap=800):
    """
...
def concatPatch(crop_w_coords, crop_h_coords, CropPatch, size=1024):
    """
